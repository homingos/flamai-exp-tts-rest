...
class MinimaxTtsService(AIService):
    ...
    def get_status(self) -> dict:
        return { "name": self.config.name, "initialized": self.is_initialized, "api_configured": bool(self.api_key and self.group_id), }
        ...
        try:
            with open(audio_path, 'rb') as audio_file:
                ...
                async with httpx.AsyncClient() as client:
                    response = await client.post(url, headers=headers, params=params, data={'purpose': 'voice_clone'}, files=files, timeout=60)
    ...
    async def generate_speech_bytes(self, text: str, voice_id: str) -> Optional[bytes]:
        ...
        params = {'GroupId': self.group_id}
        headers = {'Authorization': f'Bearer {self.api_key}', 'Content-Type': 'application/json'}
        payload = { "model": "speech-2.5-hd-preview", "text": text, "stream": False, "voice_setting": {"voice_id": voice_id}, "audio_setting": {"format": "mp3"} }
        try:
